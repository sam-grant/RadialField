...
soFile="libgm2analyses_EDM_radialField_CaloBeamPositionPlots_module.so"
...
# Split into lists with 180 each (and submit one of these each time)
rm -f SplitFileList*
split FileList.txt -l 180 -a 3 -d SplitFileList
...
EOF
  ...
  jobsub_submit -N 1 -G gm2 --OS=SL7 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --role=Analysis file://${pnfsOutDir}/runJob${fileNum}.sh
