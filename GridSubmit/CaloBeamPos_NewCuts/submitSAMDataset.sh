...
while read file; do # in `cat SAMFileList.txt`; do 
  ...
  pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/BrAna/CaloBeamPos_NewCuts/${dataset}

  if [ ! -d $pnfsOutDir ]; then
    mkdir -p $pnfsOutDir 
    chmod -R g+w $pnfsOutDir
    echo "Output files will appear in base directory $pnfsOutDir"
  fi

  # Check that the file hasn't already been created
  if [ -f ${pnfsOutDir}/${run}/caloBeamPositionPlots_${run}.${subrun}.root ]; then
    echo "Output file caloBeamPositionPlots_${run}.${subrun}.root already exists, skipping..."
    continue
  fi
